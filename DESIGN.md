...
the first 8 bytes of a sha256 hash of the public part of this key. It is the
[Communicator's](#server-communicator) responsibility to verify the identity of
clients. The recommended approach is to communicate over TLS using the client's
Fleetspeak key as TLS client identification. The
...
forwarded verbatim, and when integrating a service a developer should choose
...
package which defines a service which simple saves all received messages to text
files.
...
The Fleetspeak Client is a small process which runs on the endpoint and
...
Fleetspeak, it should be possible to include any needed additional support for
specialized tunneling or proxying.
...
[messages](#messages) to and from it. This gives full control over what is sent
to the server, but requires more integration. The sub-process should use the
