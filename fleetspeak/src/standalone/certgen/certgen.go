...
func createCACert(conf *saspb.StandaloneServerConfig) bool {
	s, err := os.Stat(conf.CaCert)
	if err == nil {
		if s.Mode().IsRegular() {
			// We don't need to create a cert - it will be opened
			// and validated later.
			log.Infof("Using existing CA cert file: %v", conf.CaCert)
			return false
		}
		log.Exitf("Existing path %v is not a regular file: %v", conf.CaCert, s.Mode())
	}
	// If the file doesn't exist, we will create it. But if we have any other error
	// something odd is going on.
	if !os.IsNotExist(err) {
		log.Exitf("Unable to stat %v: %v", conf.CaCert, err)
	}
	...
	if err := ioutil.WriteFile(conf.CaCert, c, 0644); err != nil {
		log.Exitf("Unable to write CA cert file [%v]: %v", conf.CaCert, err)
	}
	...
	if err := ioutil.WriteFile(conf.CaKey, k, 0600); err != nil {
		log.Exitf("Unable to write CA key file [%v]: %v", conf.CaKey, err)
	}
	return true
}
...
// found.
func createServerCert(conf *saspb.StandaloneServerConfig) {
	s, err := os.Stat(conf.ServerCert)
	if err == nil {
		if s.Mode().IsRegular() {
			return
		}
		log.Exitf("Existing path %v is not a regular file: %v", conf.ServerCert, s.Mode())
	}
	// If the file doesn't exist, we will create it. But if we have any other error
	// something odd is going on.
	if !os.IsNotExist(err) {
		log.Exitf("Unable to stat %v: %v", conf.ServerCert, err)
	}
	if *serverCommonName == "" {
	...
	c := pem.EncodeToMemory(&pem.Block{Type: "CERTIFICATE", Bytes: dc})
	if err := ioutil.WriteFile(conf.ServerCert, c, 0644); err != nil {
		log.Exitf("Unable to write server cert file [%v]: %v", conf.ServerCert, err)
	}
	...
	k := pem.EncodeToMemory(&pem.Block{Type: "EC PRIVATE KEY", Bytes: dk})
	if err := ioutil.WriteFile(conf.ServerKey, k, 0600); err != nil {
		log.Exitf("Unable to write server key file [%v]: %v", conf.ServerKey, err)
	}

}

func loadCA(conf *saspb.StandaloneServerConfig) (*x509.Certificate, interface{}) {
	b, err := ioutil.ReadFile(conf.CaCert)
	if err != nil {
		log.Exitf("Unable to read CA certificate [%v]: %v", conf.CaCert, err)
	}
	cb, _ := pem.Decode(b)
	if cb == nil || cb.Type != "CERTIFICATE" {
		log.Exitf("Did not find valid pem block in CA certificate file [%v]", conf.CaCert)
	}
	cert, err := x509.ParseCertificate(cb.Bytes)
	if err != nil {
		log.Exitf("Unable to parse certificate in CA certificate file [%v]: %v", conf.CaCert, err)
	}

	kfb, err := ioutil.ReadFile(conf.CaKey)
	if err != nil {
		log.Exitf("Unable to read CA key [%v]: %v", conf.CaKey, err)
	}
	db, _ := pem.Decode(kfb)
	if db == nil {
		log.Exitf("Unable to parse PEM in CA key[%v].", conf.CaKey)
	}
	var key interface{}
	switch db.Type {
	case "RSA PRIVATE KEY":
		...
		if err != nil {
			log.Exitf("Unable to parse RSA key in CA key file [%v]: %v", conf.CaKey, err)
		}
	case "EC PRIVATE KEY":
		key, err = x509.ParseECPrivateKey(db.Bytes)
		if err != nil {
			log.Exitf("Unable to parse EC key in CA key file [%v]: %v", conf.CaKey, err)
		}
	default:
