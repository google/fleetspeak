...
func (cc *componentsInfo) start(configDir string, frontendAddress, msAddress string, numServers, numClients int) error {
	...
	for i := range numServers {
		...
		serverCmd := exec.Command("cmd/fleetspeak_server/fleetspeak_server", "-logtostderr", "-components_config", serverConfigPath, "-services_config", serverServicesConfigPath)
	...
	for i := range numClients {
		...
		cmd := exec.Command("cmd/fleetspeak_client/fleetspeak_client", "-logtostderr", "-config", linuxConfigPath)
