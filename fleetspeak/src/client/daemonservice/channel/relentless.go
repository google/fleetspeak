...
//
// TODO: Find a better place for this struct - is now used in the
// client/service package and should be in a less focused location.
...
type RelentlessChannel struct {
	...
	Out chan<- AckMessage    // Messages to send to the other process. Close to shutdown.

	i chan<- *fspb.Message // other end of In
	o <-chan AckMessage    // other end of Out

	ch  *Channel // current Channel
	fin func()   // current cleanup function for ch

	pending map[string]AckMessage
	builder Builder
}
...
func NewRelentlessChannel(b Builder) *RelentlessChannel {
	...
	ret := RelentlessChannel{
		...
		pending: make(map[string]AckMessage),
		builder: b,
	}
...
func (c *RelentlessChannel) receiveOne() (am AckMessage, newChan, shutdown bool) {
	for {
		select {
		case e := <-c.ch.Err:
...
type RelentlessAcknowledger struct {
	In <-chan AckMessage // Wraps Channel.In.
...
// a.c.In is closed.
func (a *RelentlessAcknowledger) acknowledgeLoop(out chan<- AckMessage) {
	defer a.done.Done()
	defer close(out)
	defer a.flush()
