...
func (s *fakeBatchedService) ProcessMessages(ctx context.Context, msgs []*fspb.Message) error {
...
func TestBatchedService(t *testing.T) {
	...
	if diff := cmp.Diff(wantBatches, service.batches,
		...
		t.Fatalf("unexpected batches from simulated contact (-want +got): %s", diff)
	}
}
